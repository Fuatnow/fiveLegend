...
using System.Text;
public class testVertical : tk2dUIBaseDemoController {
	...
	void CustomizeListObject( Transform contentRoot, int itemId ) {
		contentRoot.Find("rankNum").GetComponent<tk2dTextMesh>().text = "NO."+(itemId+1);
		int scoreStr = allItems [itemId].score;
		int girdNum =  allItems [itemId].girdNum;
		var girdStr = "" + girdNum;
		if(girdNum >= 1000000)
		{
			girdStr = (girdNum/1000000)+"M";
		}
		else if(girdNum >= 1000)
		{
			girdStr = (girdNum/1000)+"K";
		}
		contentRoot.Find("maxGirdNum").GetComponent<tk2dTextMesh>().text = girdStr;
		contentRoot.Find("scoreNum").GetComponent<tk2dTextMesh>().text = scoreStr+"";
		contentRoot.Find("nickName").GetComponent<tk2dTextMesh>().text = "" + allItems[itemId].name;
//		contentRoot.Find("bg").GetComponent<tk2dBaseSprite>().color = allItems[itemId].color;
		contentRoot.localPosition = new Vector3(0,-itemId * itemStride, 0);
	}
...
//		numItemsTextMesh.text = "COUNT: " + numItems.ToString();
	...
	{
		Color color = new Color32((byte)Random.Range(200, 240), (byte)Random.Range(200, 240), (byte)Random.Range(200, 240), 255);
		item.color = color;
		allItems.Add (item);
	}
	...
	public void UpdateListGraphics() {
		...
		//this line is i append
		//when the allItems data change , we need update the visible item view alseo.
		for(int i=firstCachedItem;i<firstCachedItem+cachedContentItems.Count;i++)
		{
			int index = i - firstCachedItem;
			CustomizeListObject(cachedContentItems[index], i);
		}
	}
